...
if (cluster.isMaster) {
  ...
  app.get("/update", function (req, res) {
    ...
    if (publicationIDs) {
      if (publicationIDs.indexOf(",") !== -1) {
        publicationIDs = publicationIDs.split(",");
      }
      if (
        typeof publicationIDs === "string" ||
        publicationIDs instanceof String
      ) {
  ...
  app.get("/api", function (req, res) {
    if (
      (apiDebugFlag === 1 || req.query.token === cdnResetCleanAllToken) &&
      req.query.mode
    ) {
      var metaData;
      var date;
      ...
      if (req.query.mode === "authDetails") {
        ...
        metaData = {
          "X-Container-Meta-Ghost": "Buster",
          "X-Container-Meta-super": "man",
        };

        cdnAPI.createContainer(
        ...
        metaData = {
          "X-Remove-Container-Meta-Ghost": "Buster",
          "X-Container-Meta-Iron": "man",
        };

        cdnAPI.setUpdateDeleteContainerMetaData(
          req.query.containerName,
          metaData,
          function (data) {
            outputDataJSON(data, res);
        ...
        cdnAPI.deleteMultipleObjects(
          req.query.containerName,
          req.query.objectNames.split(","),
          function (data) {
            outputDataJSON(data, res);
          }
        );
      } else if (req.query.mode === "deleteAllObjectsInContainer") {
        cdnAPI.deleteAllObjectsInContainer(
          req.query.containerName,
          function (data) {
            outputDataJSON(data, res);
          }
        ...
        metaData = {
          "X-Remove-Object-Meta-Ikea": "kitchen catalog",
          "X-Object-Meta-catalog": "ikea",
        };

        //date = new Date("June 12, 2013 16:1:00"); // Your timezone!
        date = new Date().addHours(5);

        cdnAPI.updateObjectMetaData(
          req.query.containerName,
          req.query.objectName,
          metaData,
          date,
        ...
        cdnAPI.cdnEnabledContainerDetails(
          req.query.containerName,
        ...
        if (req.query.cdnEnable) {
          cdnEnable = req.query.cdnEnable;
        }
        if (req.query.logRetention) {
          logRetention = req.query.logRetention;
        }

        cdnAPI.changeCDNContainerAttributes(
          containerName,
          ttl,
          cdnEnable,
          logRetention,
