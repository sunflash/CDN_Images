...
function overWriteWithFreshData(
  activeCatalogsData,
  activeCatalogsCount,
  callback
) {
...
function cleanUnusedExpiredData(callback) {
  client.SDIFF(
    oldCatalogPubIDKey,
    activeCatalogPubIDKey,
    function (err, unusedExpiredCatalogs) {
      if (err) {
        callback(err);
      } else if (unusedExpiredCatalogs.length > 0) {
        for (var i = 0; i < unusedExpiredCatalogs.length; i++) {
          var unusedExpiredCatalogKey =
            catalogRedisKeyPrefix + unusedExpiredCatalogs[i];
...
exports.updateRedisData = function updateRedisData(
  activeCatalogs,
  activeCatalogsCount
) {
