...
var activeCatalogQuery =
  "SELECT id,time_start,time_stop,path,ipaper_id FROM mr_stories_external ";
activeCatalogQuery +=
  "WHERE active = 1 AND time_stop > NOW() AND time_start < NOW() ";
activeCatalogQuery +=
  "AND time_stop < DATE_ADD(NOW(), INTERVAL 1 YEAR) AND time_start > DATE_SUB(NOW(), INTERVAL 1 YEAR) ";
activeCatalogQuery += "AND path LIKE  '%minreklamedk.ipapercms.dk%'";

var catalogPageCountQuery =
  "SELECT COUNT(*) AS pageCount FROM mr_story_pages WHERE id = ?";
            ...
            connection.query(activeCatalogQuery, function (err, rows) {
              if (err) {
                ...
                if (rows.length > 0) {
                  ...
                  callback("Error, NO active catalogs");
                }
              }
            });
          }
    ...
    function (err, activeCatalogArray) {
      ...
      if (err) {
        ...
        if (activeCatalogArray.length > 0) {
          ...
          downloadImages.downloadActiveCatalogImage(
            activeCatalogs,
            activeCatalogArray.length
          );
