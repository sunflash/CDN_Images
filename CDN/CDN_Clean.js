...
function getContainerWithExpiredPubID(cleanPubID, callback) {
  ...
  async.each(
    cleanPubID,
    function (expirePubID, next) {
      client.KEYS(
        cdnRedisKeyPrefix + expirePubID + ".*",
        function (err, expiredCDNContainerKey) {
          if (err) {
            next(err);
          } else if (
            expiredCDNContainerKey &&
            expiredCDNContainerKey.length > 0
          ) {
            expiredContainerKeys = expiredContainerKeys.concat(
              expiredCDNContainerKey
            );
            next();
          } else {
...
function disableExpiredContainerInCDN(
  cleanPubID,
  expiredContainerInfo,
  callback
) {
  if (expiredContainerInfo && expiredContainerInfo.length > 0) {
    async.each(
      expiredContainerInfo,
      function (expiredContainerKey, next) {
    ...
    async.each(
      ...
      function (err) {
        ...
        if (err) {
          ...
          callback(null, expiredContainerInfo);
        }
      }
    );
  } else {
...
var cleanFlow = async.compose(
  cleanExpiredDataInRedisDB,
  deleteExpiredContainer,
  disableExpiredContainerInCDN,
  getContainerWithExpiredPubID
);

exports.cleanExpiredDataInCloudFileAndCDN =
  function cleanExpiredDataInCloudFileAndCDN(callback) {
      ...
      if (err) {
        ...
        cleanFlow(cleanPubID, function (err, result) {
          if (err) {
            ...
            console.log(
              "Remove " + result.length + " containers with expired data in CDN"
            );
            callback(result);
          }
        });
    ...
    console.log(
      "***** Clean old data in Cloud File and CDN ***** " + new Date()
    );
  };
...
exports.cleanCloudFileAndCDNCatalogData =
  function cleanCloudFileAndCDNCatalogData(cleanPubID, callback) {
    var cleanCloudFileAndCDNCatalogDataFlow = async.compose(
      cleanCatalogDataInRedisDB,
      deleteExpiredContainer,
      disableExpiredContainerInCDN,
      getContainerWithExpiredPubID
    );
...
exports.cleanAllCloudFileAndCDNCatalogData =
  function cleanAllCloudFileAndCDNCatalogData(callback) {
    var cleanAllCloudFileAndCDNCatalogDataFlow = async.compose(
      deleteContainer,
      disableContainer,
      getAllContainer
    );
...
exports.removeEmptyCDNContainer = function removeEmptyCDNContainer(callback) {
  var removeEmptyCDNContainerFlow = async.compose(
    deleteContainer,
    disableContainer,
    getEmptyContainer
  );
