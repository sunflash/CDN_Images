...
function deleteLocalCatalogData(publicationIDs, callback) {
  async.each(
    publicationIDs,
    function (publicationID, next) {
      var catalogImageDirectory = path.join(
        saveFilePathPrefix,
...
exports.updateCatalogData = function updateCatalogData(
  publicationIDs,
  callback
) {
  var updateCatalogDataFlow = async.compose(
    cleanCatalogCDNData,
    deleteLocalCatalogData
  );
