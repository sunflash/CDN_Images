...
function checkImageExistLocal(parameters, callback) {
  ...
  fs.exists(imageFilePath, function (exists) {
    if (!exists) {
      callback("Image NOT exist");
    } else {
      callback(null, parameters, path.dirname(imageFilePath));
    }
  });
}

function createResizeImageFolderIfNotExist(
  parameters,
  imageFolderPath,
  callback
) {
  var saveFileFolderPath = path.join(
    imageFolderPath,
...
var resizeImageFlow = async.compose(
  resizeRequestImage,
  createResizeImageFolderIfNotExist,
  checkImageExistLocal
);

exports.resizeImage = function resizeImage(resizeParameters, callback) {
  var resizeImagePath = path.join(
    saveFilePathPrefix,
    resizeParameters.PublicationID.toString(),
    resizeParameters.Width + "x" + resizeParameters.Height,
    resizeParameters.PageNumber + ".jpg"
  );
